...
import readfile
from generator_utils import log_statistics, save_cache, query_dbpedia, strip_brackets, encode
...
def extract_bindings(data, template):
    ...
    for match in best_matches:
        ...
        for variable in variables:
            ...
            # print json.dumps(binding[variable], indent=4)
...
def build_dataset_pair_v2(binding, template):
    english = getattr(template, 'question')
    sparql = getattr(template, 'query')
    for variable in binding:
        uri = binding[variable]['uri']
    ...
    sparql = encode(sparql)
    dataset_pair = {'english': english.replace("\"", ""), 'sparql': sparql.replace("\"", "")}
    # print dataset_pair
    return dataset_pair
...
def generate_dataset(templates, output_dir, file_mode):
    ...
    it = 0
    with open(output_dir + '/data_300.en', file_mode) as english_questions, open(output_dir + '/data_300.sparql',
                                                                                 file_mode) as sparql_queries:
        for template in templates:
            ...
            try:
                ...
                filename = getattr(template, 'file_csv').replace("\"", "")

                print "##############################"
                print "Filename: {}".format(filename)
                generate_query = getattr(template, 'generate_query')
                print "Generate query: {}".format(generate_query)

                bindings, count = readfile.extract_bindings_v2(generate_query)

                print "Count: {}".format(count)
                print "##############################"

                # bindings = readfile.extract_bindings(filename)
                ...
                for binding in bindings:
                    dataset_pair = build_dataset_pair_v2(binding, template)
                    # print "x", det_pair
                    if (dataset_pair):
                        ...
                        # print a.group(1),b.group(1), c.group(1)
                        a = a.group(1)
                        b = b.group(1)
                        c = c.group(1)
    ...
    for i, variable in enumerate(variables):
        ...
        if variable_has_a_type and add_type_requirements:
            ...
            if variable_is_subclass(generator_query, variable):
                generator_query = add_requirement(generator_query, SUBCLASS_REPLACEMENT.format(variable=variable,
                ...
                if normalized_target_class in SPECIAL_CLASSES and do_special_class_replacement:
                    ...
                    generator_query = add_requirement(generator_query,
                                                      CLASSES_REPLACEMENT.format(variable=variable, classes=classes))
                else:
                    ontology_class = normalized_target_class
                    generator_query = add_requirement(generator_query, CLASS_REPLACEMENT.format(variable=variable,
...
if __name__ == '__main__':
    ...
    requiredNamed.add_argument('--output', dest='output', metavar='outputDirectory', help='dataset directory',
                               required=True)
    ...
    used_resources = collections.Counter(
        json.loads(open(resource_dump_file).read())) if use_resources_dump else collections.Counter()
    file_mode = 'a' if use_resources_dump else 'w'
    templates = readfile.read_template_file(template_file)
    print len(templates)
